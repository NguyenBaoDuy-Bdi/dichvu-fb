...
function coppy(element) {
    ...
    swa1(`Sao chép thành công ` + range, "success");
...
function createDataTable(element, url, columns, order = []) {
    ...
    $(document).ready(function () {
        $(element).DataTable({
            ...
            serverSide: true,
            pageLength : 10,
            responsive: false, // Tắt tính năng responsive mặc định
                scrollX: true, // Cho phép thanh trượt ngang
                scrollY: true, // Thiết lập chiều cao tối đa của bảng và cho phép thanh trượt dọc
              ajax: {
                url: url,
                type: "POST",
                error: function (xhr, error, code) {
                    ...
                    console.log(error);
                    console.log(code);
                    // Có thể thực hiện xử lý lỗi tại đây
...
function orderSpeed(id){
    Swal.fire({
        title: 'Xác nhận tăng tốc đơn',
        text: 'Bạn có chắc chắn muốn tăng tốc đơn hàng này?',
        icon: 'info',
        showCancelButton: true,
        confirmButtonText: 'Đồng ý',
        cancelButtonText: 'Hủy',
