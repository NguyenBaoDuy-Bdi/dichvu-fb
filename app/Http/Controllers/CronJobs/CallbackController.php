...
class CallbackController extends Controller
{
    public function telegramV1(Request $request)
    {
        ...
        if (isset($input['message'])) {
            ...
            elseif (strpos($text, '/xacthuc') !== false) {
                $token = str_replace('/xacthuc ', '', $text);
        ...
        if (isset($input['callback_query'])) {
            ...
            if ($data == 'verify_account') {
                $img = storage_path('/app/images/cach-lay-api-token.png');
                $bot->sendPhoto([
                    'chat_id' => $chat_id,
                    'photo' => $img,
                    'caption' => "Bạn vui lòng truy cập vào thông tin tài khoản của bạn để lấy token API \n(Truy cập https://" . getDomain() . "/account/profile) \nLưu ý: sau khi đã coppy Token bạn vui lòng dùng lệnh /xacthuc <token> để xác thực tài khoản",
                ]);
            }
        }
    }
